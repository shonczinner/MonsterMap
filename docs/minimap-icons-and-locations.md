...
- `Worldmap.ts` packs this file into `worldmap.jag` as `labels.dat`:
  ...
  `rs2b0t/src/mapview/MapView.ts` reads `labels.dat` (`mapLabelX/Y` = those
...
- The id indexes a 0-based legend of symbol names:
  `rs2b0t/src/mapview/worldmapKeyNames.ts` (`WORLDMAP_KEY_NAMES`, 49 entries,
  `0..48`). Examples: `5 → "Bank"`, `8 → "Mining Site"`, `26 → "Fishing Spot"`,
...
- `lib/icons.ts` — export `WORLDMAP_KEY_NAMES` (copied from
  `rs2b0t/src/mapview/worldmapKeyNames.ts`) so the legend + `mapfunction → name`
  lookup live in one place. (It lives in **rs2b0t**, not `Server`; MonsterMap
  should keep its own copy to avoid a runtime dependency on rs2b0t's build.)
